...
namespace lms.buildingblocks.middleware
{
    ...
    public class RedisRateLimitingMiddleware
    {
        ...
        private async Task<bool> FixedWindowRateLimit(string clientKey)
        {
            ...
            _ = await _redisDb.KeyExpireAsync(windowKey, TimeSpan.FromSeconds(_configuration.WindowSeconds));
        ...
        private async Task<bool> SlidingWindowRateLimit(string clientKey)
        {
            ...
            _ = await _redisDb.SortedSetAddAsync(windowKey, now.ToUnixTimeMilliseconds().ToString(), now.ToUnixTimeMilliseconds());

            // Remove timestamps outside the window
            long oldestAllowedTime = now.AddSeconds(-_configuration.WindowSeconds).ToUnixTimeMilliseconds();
            _ = await _redisDb.SortedSetRemoveRangeByScoreAsync(windowKey, 0, oldestAllowedTime);
        ...
        private async Task<BucketState> GetBucketState(string bucketKey)
        {
            ...
            {
                // Explicitly convert to string to avoid overload ambiguity
                string json = (string)serializedState!;
                BucketState? state = JsonSerializer.Deserialize<BucketState>(json);
                if (state != null)
                {
                    return state;
                }
        ...
        private async Task SaveBucketState(string bucketKey, BucketState state)
        {
            ...
            _ = await _redisDb.StringSetAsync(bucketKey, serializedState, TimeSpan.FromSeconds(_configuration.WindowSeconds));
        }
